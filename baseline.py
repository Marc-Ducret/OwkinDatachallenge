...
def main():
    ...
    filenames_train = [
        train_dir / "ID_{:03d}.npy".format(idx) if
        (train_dir / "ID_{:03d}.npy".format(idx)).is_file() else
        train_dir / "ID_{:03d}_annotated.npy".format(idx)
        for idx in train_output["ID"]
    ]
    for filename, idx in zip(filenames_train, train_output["ID"]):
